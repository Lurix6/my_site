...
class NewPostElement extends React.Component{
        ...
        selectedImgForPostImg = (selectedImg) => {
            this.setState({selectedImg:selectedImg, openPhotoDialog: false, postCategory: 'img'})
        ...
        handleAddNewPost = (selectedAccount) => {
          if(this.state.text){
              this.props.addNewPost({id: selectedAccount.posts.length, author: selectedAccount.id ,selectedImg: this.props.fillListImg[this.state.selectedImg].img, text: this.state.text, date: new Date, whoLike: [], postCategory: 'graffiti'})
              this.setState({postCategory: 'simple', selectedImg: 0, row: 1, text: ''})
          }
        }
        handleAddNewPostImg = (selectedAccount) => {
              this.props.addNewPost({id: selectedAccount.posts.length, author: selectedAccount.id ,selectedImg: this.state.selectedImg, text: this.state.text, date: new Date, whoLike: [], postCategory: 'img'})
              this.setState({postCategory: 'simple', selectedImg: 0, row: 1, text: ''})
        }

        handleAddSimplePost = (selectedAccount) => {
          if(this.state.text){
              this.props.addNewPost({id: getFormatedDateId().toString(), author: selectedAccount.id ,text: this.state.text, date: new Date, whoLike: [], postCategory: 'simple'})
              this.setState({text: '', postCategory: 'simple', selectedImg: 0})
          }
        }
    ...
    render() {
        ...
        const ListImg = <div className='scrolls'>
                ...
                return(<div>
                          {this.state.postCategory == 'simple' ?
                            <div className={classes.paper + ' ' + classes.paper_row}>
                                      ...
                                      <div style={{paddingLeft: '20px', borderLeft: '1px solid #e7e8ec',  marginLeft:'10px'}} >
                                          {this.state.text.length > 0 ?
                                            <Button variant="contained" color="primary" onClick={() =>this.handleAddSimplePost(selectedAccount)}>
                                             ...
                                             <div style={{width:'24px', height:'24px', borderRadius: '50%'}} onClick={() => {this.setState({postCategory: 'fraffiti'})} }>
                              ...
                              :(
                                (this.state.postCategory === 'img' && typeof(this.state.selectedImg) == 'string' ) ?
                                ...
                                <div className={classes.paper}>
                                      ...
                                      <div className={classes.paper} style={{display: 'flex', justifyContent: 'center'}} >
                                          <div style={{  display: 'block',  float: 'left' ,position: 'relative'}} >
                                              <img style={{width: 'auto', height: 'auto', maxWidth: '100%', maxHeight: '100%'}} src={this.state.selectedImg} alt=""/>
                                              <div onClick={() => this.setState({postCategory: 'simple', selectedImg: 0})} style={{backgroundColor: 'black', opacity: '0.7' ,position: 'absolute' , width: '25px', height: '25px', top: 0, right: 0}} >
                                                  <Icon className={classes.titleTextFormat}>close</Icon>
                                              </div>
                                          </div>
                                      </div>
                                      <div style={{width: '100%', display: 'flex', justifyContent: 'flex-end'}} >
                                            <Button variant="contained" color="primary" onClick={() =>this.handleAddNewPostImg(selectedAccount)}>
                                               Надіслати
                                             </Button>
                                      </div>
                                  </div>
                                ...
                                :(
                                    (this.state.postCategory === 'img' && typeof(this.state.selectedImg) == 'object' ) ?
                                    ...
                                    <div className={classes.paper}>
                                          ...
                                          <div className={classes.paper} style={{display: 'flex', justifyContent: 'center', justifyContent: 'center'}} >
                                              <div style={{  display: 'flex' , flexWrap: 'wrap'}} >
                                                  {this.state.selectedImg.map((el, index) =>
                                                    <div style={{float: 'left' ,position: 'relative'}} key={index}>
                                                         <img style={{width: '160px', height: '160px', maxWidth: '100%', maxHeight: '100%'}} src={el} alt=""/>
                                                          <div onClick={() => this.deleteImgToState(el)} style={{backgroundColor: 'black', opacity: '0.7' ,position: 'absolute' , width: '25px', height: '25px', top: 0, right: 0}} >
                                                              <Icon className={classes.titleTextFormat}>close</Icon>
                                                          </div>
                                                    </div>
                                                  )}
                                    ...
                                    <div style={{position: 'relative'}}>
                                        ...
                                        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                            <Icon style={{color: 'grey', cursor: 'pointer' ,fontSize: '30px', margin: '0 10px 0 10px'}} onClick={() => this.setState({postCategory: 'simple'})}>close</Icon>
                                            {ListImg}
                              ...
                              <AddPhotoDialog selectedImgForPostImg={this.selectedImgForPostImg} open={this.state.openPhotoDialog} clouseDialog={this.handleClickClousePhotoDialog} tileData={selectedAccount.photo} />
                            </div>
                            )
                            }
